...
static std::string extract_stream_content(const std::string& line) {
    ...
    if (!Json::parseFromStream(reader, json_stream, &root, &errors)) return "";
...
size_t deepseek::WriteCallback(void* contents, size_t size, size_t nmemb, std::string* data) {
 ...
 */
deepseek::deepseek(const std::string& key, bool is_stream) : api_key(key), is_stream(is_stream) {
    if (key.empty()) {
 ...
 */
std::string deepseek::send_request(const std::string& model, const std::string role, const std::string& data) {
...
*/
bool deepseek::add_message(const std::string& role, const std::string& content) {
    if (role.empty() || content.empty()) {
        #ifdef DEBUG
        std::cerr << "Invalid message: role or content is empty." << std::endl;
        std::cerr << "Role: '" << role << "', Content: '" << content << "'" << std::endl;
        #endif
 ...
 */
std::string deepseek::ask(const std::string& model, const std::string& question, bool multi_turn) {
